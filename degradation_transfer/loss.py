...
from utils import create_penalty_mask, map2tensor
...
class SumOfWeightsLoss_multichannel(nn.Module):
    """ Encourages the kernel G is imitating to sum to 1 """

    def __init__(self):
        super(SumOfWeightsLoss_multichannel, self).__init__()
        self.loss = nn.L1Loss()

    def forward(self, kernel):
        return self.loss(torch.sum(torch.ones(3).to(kernel.device)).unsqueeze(0), torch.sum(kernel).unsqueeze(0))


class CentralizedLoss(nn.Module):
    """ Penalizes distance of center of mass from K's center"""

    def __init__(self, k_size, scale_factor=1.0):
        ...
        self.center = Variable(torch.FloatTensor([wanted_center_of_mass, wanted_center_of_mass]).unsqueeze(-1).cuda(), requires_grad=False)
        self.loss = nn.MSELoss()
